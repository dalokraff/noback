...
MyNews.destroy = function (self)
	self._peddler_stock = nil
	self._chips = nil
	self._app_prices = nil
end

MyNews.get_peddler_stock = function (self)
	return self._peddler_stock
end

local empty_params = {}

MyNews.get_filtered_items = function (self, filter, params)
...
MyNews.get_chips = function (self, chip_type)
	return self._chips[chip_type]
end

MyNews.get_app_price = function (self, app_id)
	return self._app_prices[app_id]
end

MyNews.get_steam_item_price = function (self, steam_itemdefid)
	return self._steam_item_prices[steam_itemdefid], self._steam_item_currency
end

MyNews.is_purchaseable = function (self, steam_itemdefid)
	return self._steam_item_prices[steam_itemdefid] ~= nil
end

MyNews.get_unseen_currency_rewards = function (self)
...
MyNews.refresh_stock = function (self, external_cb)
	self._peddler_stock = {}
...
MyNews._refresh_stock_cb = function (self, external_cb, result)
	...
	for i = 1, #stock do
		...
		if item.ItemId and not rawget(ItemMasterList, item.ItemId) then
			printf("MyNews - ItemMasterList has no item %q", tostring(item.ItemId))
		else
	...
	print(string.format("[MyNews] _refresh_stock_cb -> Added %s item(s) to the peddler stock", #peddler_stock))
...
MyNews.set_chips = function (self, chip_type, chip_amount)
	self._chips[chip_type] = chip_amount
end

MyNews.refresh_chips = function (self, external_cb)
	local request = {
...
MyNews._refresh_chips_cb = function (self, external_cb, result)
...
MyNews.refresh_layout_override = function (self, use_mirrored_title_data, external_cb)
	local mirror = self._backend_mirror
...
MyNews._refresh_layout_override_cb = function (self, external_cb, result)
...
MyNews.store_display_items = function (self)
	local mirror = self._backend_mirror
	local title_data = mirror:get_title_data()
...
MyNews.refresh_platform_item_prices = function (self, external_cb)
	if HAS_STEAM then
		print("[MyNews] refresh steam item prices")
		Managers.steam:request_item_prices(callback(self, "_refresh_steam_item_prices_cb", external_cb))
	end
end

MyNews._read_bundle_from_steam = function (self, steam_itemdefid)
...
MyNews._refresh_steam_item_prices_cb = function (self, external_cb, price_list, currency)
...
MyNews.refresh_app_prices = function (self, external_cb)
...
MyNews._refresh_app_prices_steam = function (self, external_cb)
	local request = {
...
MyNews._refresh_app_prices_steam_cb = function (self, external_cb, result)
	local function_result = result.FunctionResult
	local success = true

	if function_result.error then
		print("[MyNews] _refresh_app_prices_steam_cb ERROR", function_result.error)
...
MyNews._refresh_app_prices_psn = function (self, external_cb)
...
MyNews._refresh_app_prices_psn_cb = function (self, external_cb, product_label_lookup, result_json)
	print("")
...
MyNews._refresh_app_prices_xboxlive = function (self, external_cb)
...
MyNews._refresh_app_prices_xboxlive_cb = function (self, external_cb, product_id_lookup, result)
...
MyNews.exchange_chips = function (self, item_id, chip_type, expected_chip_amount, external_cb)
	local request = {
		StoreId = PEDDLER_ID,
		ItemId = item_id,
...
MyNews._exchange_chips_success_cb = function (self, external_cb, result)
	...
	for i = 1, #items do
		...
		print(string.format("[MyNews] Exchanged %s %s for %s", chip_amount, chip_type, item.ItemId))
	end
...
MyNews._exchange_chips_error_cb = function (self, external_cb, result, reenable_queue_function)
...
MyNews._store_purchase_made_cb = function (self, items, result)
	local function_result = result.FunctionResult
	...
	if updated_statistics then
		...
		if not player or not statistics_db then
			print("[MyNews] Could not get statistics_db, skipping updating statistics...")
		else
			...
			for key, value in pairs(updated_statistics) do
				if not statistics_db.statistics[player_stats_id][key] then
					Application.warning("[MyNews] updated_statistics " .. key .. " doesn't exist.")
				else
...
MyNews._refresh_login_rewards_cb = function (self, external_cb, result)
...
MyNews.get_login_rewards = function (self)
	-- return self._login_rewards
    return {
        next_claim_timestamp = 10,
    }
end

MyNews.done_claiming_login_rewards = function (self)
	return self._is_done_claiming
end

MyNews.claim_login_rewards = function (self, external_cb, offset)
...
MyNews._claim_store_rewards_cb = function (self, external_cb, offset, result)
