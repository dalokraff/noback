...
MyCommon.can_wield = function (self, career_name, item_data)
	local can_wield = item_data.can_wield

	assert(can_wield, "MyCommon - Item %q has not specified what profiles that can use it.", item_data.name or "(item_data missing name)")
...
MyCommon.filter_postfix_cache = MyCommon.filter_postfix_cache or {}

local empty_params = {}

MyCommon.filter_items = function (self, items, filter_infix, params)
	local filter_postfix = MyCommon.filter_postfix_cache[filter_infix]

	if not filter_postfix then
		filter_postfix = self:_infix_to_postfix_item_filter(filter_infix)
		MyCommon.filter_postfix_cache[filter_infix] = filter_postfix
	end
...
MyCommon._infix_to_postfix_item_filter = function (self, filter_infix)
	local output = {}
	local stack = {}
...
MyCommon.serialize_traits = function (self, traits)
...
MyCommon.serialize_runes = function (self, runes)
...
MyCommon.commit_load_time_data = function (self, load_time_data)
